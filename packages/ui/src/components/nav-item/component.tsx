...
>;
const TextBase = Text as React.ComponentType<
...
export function NavItem({
  ...
  const explicitLabelVisibility = showLabel ?? labelVisibleProp;
  const labelVisible =
    explicitLabelVisibility ?? (placement === "sidebar" || Boolean(active));
  ...
  return (
    ...
    >
      {icon}
      {labelVisible ? (
        <TextBase className={cn(navLabelVariants({ placement, active }))}>
          {label}
        </TextBase>
      ) : null}
    </PressableBase>
